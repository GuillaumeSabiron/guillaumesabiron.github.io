(() => {
  ...
  ready(() => {
    ...
    document.querySelectorAll('[data-video-button]').forEach((button) => button.addEventListener('click', () => {
      ...
      container.innerHTML = '<iframe src="https://www.youtube-nocookie.com/embed/qd4ZlmutXas?autoplay=1" title="' + button.dataset.videoTitle + '" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
      dialog.showModal();
    }));
    const closeVideo = () => {
      if (!dialog) return;
